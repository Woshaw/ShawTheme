...
<div id="comments" class="comments-area area">
    ...
    <?php if ( have_comments() ) : ?>
        <h3 class="comments-title">
            <?php printf( _x( '%s Comments','Used in comment list title', 'shawtheme' ), number_format_i18n( get_comments_number() ) ); ?>
        </h3>
    ...
    if ( !comments_open() && get_comments_number() ) {
        ...
        printf( '<p class="no-comments">%s</p>', __( 'Comments are closed.', 'default' ) );
    ...
    $textholder =  have_comments() ? __( 'Enter your comment here ...', 'shawtheme' ) : __( 'No comments yet, be the first to comment !', 'shawtheme' );
    comment_form(
        array(
            'fields'               => $fields,
            'comment_field'        => '<p class="comment-form-comment"><label class="screen-reader-text" for="comment">' . _x( 'Comment', 'Used before comment textarea.', 'shawtheme' ) . '</label><textarea id="comment" name="comment" rows="8" placeholder="' . $textholder . '" aria-required="true"></textarea></p>',
